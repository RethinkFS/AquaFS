...
AquaFS 是一个以 ZenFS 为原型的适用于 Zoned Storage SSD 的文件系统，它将原来的 ZenFS 模块化，扩展原有的应用场景，添加 RAID 来平衡数据安全和写放大，同时添加调参模块以提高文件系统的智能性和性能。
...
| 基于 Zone 的智能动态分区 RAID | 全盘和分区 RAID 模式<br />整体进度约80% | 1️⃣ 全盘 RAID 模式 RAID0、RAID1、RAID-C<br/>2️⃣ 分区 RAID 模式 Zone 映射和 RAID 逻辑分配<br />3️⃣ 数据完整性检测和恢复 |
| 异步 IO 优化 |  |  |
